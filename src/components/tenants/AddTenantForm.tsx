...
export interface AddTenantFormProps {
  onSuccess?: () => void;
  onCancel?: () => void;
  /** When false, primary actions are hidden (e.g. embedded in a dialog that supplies its own footer). */
  showFooter?: boolean;
}
    ...
    <div className="grid gap-4">
      {selectedPg ? (
        <p className="text-sm text-muted-foreground">
          Adding to <span className="font-medium text-foreground">{selectedPg.name}</span> (change PG from the header if needed).
        </p>
      ) : null}
      ...
      <div className="space-y-2">
        <Label>Monthly rent & add-ons (AC / Cooler / Geyser)</Label>
        <div className="flex flex-wrap gap-2">
          <Input type="number" min={0} placeholder="Rent" value={form.monthlyRent} onChange={(e) => update("monthlyRent", e.target.value)} className="min-w-[100px] flex-1" />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-2">
          ...
          <Input type="number" min={1} placeholder="e.g. 1" value={form.bedNumber} onChange={(e) => update("bedNumber", e.target.value)} />
        </div>
      </div>
      <div className="space-y-2">
        <Label>Preferred rent collection</Label>
      ...
      {showFooter ? (
        <div className="flex flex-wrap justify-end gap-2 border-t pt-4">
          ...
          <Button type="button" onClick={handleInviteTenant} disabled={!canSubmit}>
